...
public class TimingInputHandler {
    ...
    private final static String timeRegex = "^([0-9]|0[0-9]|1[0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$";
    ...
    private Time handleTimeInput(String time){
        if(!time.matches(timeRegex)){
            ...
            log.warn("The time specified in args didn't match the format we needed (HH:mm:ss) so defaulting to right now : "+ nowTime);
            return nowTime;
        }
        Time rightInputTime = Time.valueOf(time);
        log.info("Setting Time to : " + rightInputTime);
